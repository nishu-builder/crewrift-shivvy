...
# Shivvy: verbose play-phase tracing for headless diagnosis. On by default;
# set NISHAD_VERBOSE=0 to silence. See docs/ITERATION.md.
let shivvyVerbose = getEnv("NISHAD_VERBOSE", "1") != "0"
...
type
  ...
  Bot = ref object
    ...
    taskHoldIndex: int
    frameTick: int
    traceLastIntent: string
    traceLastTick: int
    traceLastRole: BotRole
    centerMicros: int
...
proc selfColorName(bot: Bot): string =
  ## Returns this bot's own color name for log lines.
  playerColorName(bot.selfColorIndex)

proc traceFrame(bot: var Bot, mask: uint8) =
  ## Emits a tick-stamped trace whenever the bot's intent or role changes,
  ## plus a heartbeat every 50 ticks so long task/hunt stretches stay visible.
  ## bot.intent is already set throughout decideNextMask, so this surfaces the
  ## entire play phase (movement, tasks, imposter hunt/kill, death) in the
  ## headless policy_agent logs that `coworld episode-logs` returns.
  if not shivvyVerbose:
    return
  let changed =
    bot.intent != bot.traceLastIntent or bot.role != bot.traceLastRole
  if not changed and bot.frameTick - bot.traceLastTick < 50:
    return
  bot.traceLastIntent = bot.intent
  bot.traceLastTick = bot.frameTick
  bot.traceLastRole = bot.role
  echo "[t=", bot.frameTick, "][", bot.selfColorName, "][", roleName(bot.role),
    (if bot.isGhost: "/ghost" else: ""), "] (", bot.playerWorldX, ",",
    bot.playerWorldY, ") mv=", movementName(mask), " | ", bot.intent
...
proc decideNextMask(bot: var Bot): uint8 =
  ## Traces every decision for headless diagnosis, then returns the chosen mask.
  result = bot.decideNextMaskInner()
  bot.traceFrame(result)
